...
import { PRODUCT_FILTERS, PRODUCT_SORTS } from '../../utils/productListFilters';
import { PRODUCT_FILTER_LABEL_KEYS, PRODUCT_SORT_LABEL_KEYS } from '../../utils/filterLabelKeys';
import { useTranslation } from '../../i18n/useTranslation';

export default function ProductFilterSortModal({
  visible,
  filter,
  sort,
  onClose,
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
            {PRODUCT_FILTERS.map((key) => {
              ...
              return (
                ...
                >
                  {t(PRODUCT_FILTER_LABEL_KEYS[key])}
                </Chip>
              );
            })}
          </View>
            ...
            <Card.Content style={{ paddingVertical: 4, paddingHorizontal: 0 }}>
              <RadioButton.Group onValueChange={setDraftSort} value={draftSort}>
                {PRODUCT_SORTS.map((key) => (
                  <RadioButton.Item
                    key={key}
                    label={t(PRODUCT_SORT_LABEL_KEYS[key])}
                    value={key}
                    position="leading"
