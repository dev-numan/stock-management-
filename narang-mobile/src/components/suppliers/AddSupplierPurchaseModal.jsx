...
export default function AddSupplierPurchaseModal({
  visible,
  supplierName,
  onSubmit,
  onClose,
  ...
  const handleSubmit = () => {
    ...
    if (!value || value <= 0) {
      setError(t('supplier.purchaseAmount'));
      return;
    }
    setError(null);
  ...
  return (
    <Portal>
      <Dialog visible={visible} onDismiss={onClose} style={{ borderRadius: 12 }}>
        <Dialog.Title style={textDir}>{t('supplier.addPurchaseTitle')}</Dialog.Title>
        <Dialog.Content>
          {supplierName ? (
          ...
          <AppInput
            label={t('supplier.purchaseAmount')}
            value={amount}
